...
class DQN(QN):
    ...
    def add_summary(self):
        ...
        if not self.config.batch:
            ...
            self.avg_ep_len_placeholder  = tf.placeholder(tf.float32, shape=(), name="avg_ep_len")
            self.max_ep_len_placeholder  = tf.placeholder(tf.float32, shape=(), name="max_ep_len")
            self.std_ep_len_placeholder  = tf.placeholder(tf.float32, shape=(), name="std_ep_len")
        ...
        if not self.config.batch:
            ...
            tf.summary.scalar("Avg_Episode_Len", self.avg_ep_len_placeholder)
            tf.summary.scalar("Max_Episode_Len", self.max_ep_len_placeholder)
            tf.summary.scalar("Std_Episode_Len", self.std_ep_len_placeholder)
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        if not self.config.batch:
            fd.update({
                ...
                self.avg_ep_len_placeholder: self.avg_ep_len,
                self.max_ep_len_placeholder: self.max_ep_len,
                self.std_ep_len_placeholder: self.std_ep_len,
            })
